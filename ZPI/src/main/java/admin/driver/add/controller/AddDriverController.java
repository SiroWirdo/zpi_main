...
//TODO przechwytywanie erroru że mail już użyty
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			// TODO jeżli wywala że nie jest unikalny to proces nie umiera...
			...
			if(valid && uniquePesel && uniqueLicense && uniqueUsername && validPesel && password && uniqueRegistrationNumber && uniqueSideNumber){
				...
				if(!uniquePesel){
					error += "- Taki PESEL jest już przypisany do innej osoby \n";
				}
				if(!uniqueLicense){
					error += "- Taka licencja jest już przypisana do innego kierowcy \n";
				}
				if(!uniqueUsername){
					error += "- Taki użdytkownik już istnieje \n";
				}

				if(!validPesel){
					error += "- PESEL powinien mieć 11 znaków \n";
				}
				...
				if(!password){
					error += "- Hasła nie zgadzajś się \n";
				}
				
				if(!uniqueRegistrationNumber){
					error += "- Samochód o takim numerze rejestracyjnym juź istnieje \n";
				}
